...
class ResourceManager {
    ...
    getResourcePaths() {
        ...
        const paths = {
            cssHref: this.resourceConfig.css ? (cdn ? `${basePath}/css/${this.resourceConfig.css}` : `/css/${pkg.name}-${cardType}.css`) : '',
    ...
    generateCardContent() {
        ...
        const externalScripts = externalJsSrcs.map(src => `<script src="${src}"></script>`).join('');

        return { defaultCss, defaultScripts, externalScripts };
    }
